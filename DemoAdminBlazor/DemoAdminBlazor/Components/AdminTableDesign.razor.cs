...
partial class AdminTableDesign
{
    ...
    string curd_DialogClassName = "Large";
    ...
    async Task BuildBlazorCode()
    {
        if (design.TableDescrptor == null)
        {
            await SwalService.Warning("请选择实体类型！");
            return;
        }
        ...
        //     return;
        // }
        if (await SwalService.ShowModal(new SwalOption { Title= $"即将保存文件，确认继续吗？",Content= savePath, Category= SwalCategory.Question }) == false) return;
        ...
        var ManyToOneColumns = new List<string>();

        StringBuilder code = new StringBuilder();

        code.AppendLine($"@page \"/{menuPath.Replace("_","")}\"");
        code.AppendLine($"<AdminTable TItem=\"{genOptions.AllTables[0].TypeName}\" OnBeforeQuery=\"OnBeforeQuery\" {(genOptions.curd_DialogClassName == "fullscreen" ? "EditDialogFullScreenSize = \"FullScreenSize.Always\"" : "EditDialogSize=\"Size."+ genOptions.curd_DialogClassName + "\"")} {(genOptions.Design.IsTreeNav ? "IsTree" : "ShowExportButton ShowExtendButtons IsPagination ShowSearch")} {(genOptions.curd_IsMultiSelect ? "IsMultipleSelect" : "")}>");
        code.AppendLine("    <TableColumns>");
        if (genOptions.Design.IsTreeNav)
        {
            code.AppendLine("        <TableColumn @bind-Field=\"@context.ParentId\" Visible=\"false\" IsVisibleWhenAdd=\"true\" IsVisibleWhenEdit=\"true\">");
            code.AppendLine("            <EditTemplate Context=\"v\">");
            code.AppendLine($"                <SelectTree @bind-Value=\"v.ParentId\" TValue=\"long\" Items=\"ParentTreeItems\" />");
            code.AppendLine("            </EditTemplate>");
            code.AppendLine("        </TableColumn>");
        }
        foreach (var item in genOptions.Design.ColumnDesigns)
        {
            if (item.IsDisplay)
            {
                code.AppendLine($"        <TableColumn @bind-Field=\"context.{item.Column.CsName}\"  Text=\"{item.DisplayText}\" {(!genOptions.Design.IsTreeNav ? ("Filterable=\"true\"" + (item.CanSearchText ? " Searchable=\"true\"" : "")) : "")} />");
            }
            if (item.IsDisplayManyToOne)
            {
                ManyToOneColumns.Add($".Include(a => a.{item.ManyToOne.Name})");

                code.AppendLine($"        <TableColumn @bind-Field=\"context.{item.ManyToOne.Name}Id\"  Text=\"{item.DisplayText}\" Filterable=\"true\">");
                code.AppendLine($"            <Template Context=\"v\">@v.Row.{item.ManyToOne.Name}?.{item.ManyToOne.Name}Name</Template>");
                code.AppendLine("             <FilterTemplate>");
                code.AppendLine("                <FilterProvider>");
                code.AppendLine($"                 <AdminSelectEntityFilter TItem=\"{item.ManyToOne.Name}\" GetText=\"x => x.{item.ManyToOne.Name}Name\" />");
                code.AppendLine("                </FilterProvider>");
                code.AppendLine("             </FilterTemplate>");
                code.AppendLine("        </TableColumn>");
            }
        ...
        code.AppendLine("    </TableColumns>");
        code.AppendLine("</AdminTable>");
        code.AppendLine();
        code.AppendLine("@code {");
        code.AppendLine($"    private void OnBeforeQuery(AdminQueryEventArgs<{genOptions.AllTables[0].TypeName}> e)");
        code.AppendLine("    {");
        if (ManyToOneColumns.Count > 0)
        {
            code.AppendLine($"        e.Select{string.Join("", ManyToOneColumns.ToArray())};");
        }
        code.AppendLine("    }");
        if (genOptions.Design.IsTreeNav)
        {
            code.AppendLine("    private static List<TreeViewItem<long>> ParentTreeItems { get; set; }");
        }
        code.AppendLine("}");

        var savePathDir = Path.GetDirectoryName(savePath);
        if (!Directory.Exists(savePathDir)) Directory.CreateDirectory(savePathDir);
        File.WriteAllText(savePath, code.ToString());
        await SwalService.Success("文件已保存，请检查！");
